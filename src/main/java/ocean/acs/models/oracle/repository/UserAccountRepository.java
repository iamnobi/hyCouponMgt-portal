...
public interface UserAccountRepository extends CrudRepository<UserAccount, Long>,
  JpaSpecificationExecutor<UserAccount> {
    ...
    @Query(value = "select UA.* from USER_ACCOUNT UA \n"
      + "join account_group ag on ua.id = ag.account_id\n"
      + "join USER_GROUP ug on ag.group_id = ug.id \n"
      + "where ua.issuer_bank_id = ?1 and ug.type = 'SYSTEM' and ug.scope = 'BANK' and ua.delete_flag = 0",
      nativeQuery = true)
