...
import ocean.acs.models.oracle.entity.BlackListMerchantUrl;
...
public interface BlackListMerchantUrlRepository extends CrudRepository<BlackListMerchantUrl, Long>,
    ...
    @Query(value = "select new ocean.acs.models.data_object.kernel.KernelBlackListMerchantUrlDO(b.url, b.transStatus) "
    ...
    @Query(value = "select b from BlackListMerchantUrl b where b.id in ?1 and b.deleteFlag=0")
    List<BlackListMerchantUrl> findByIdIn(List<Long> ids, Pageable pageable);

    @Transactional
    @Modifying
    @Query("update BlackListMerchantUrl m set m.deleteFlag = 1, m.deleter = :deleter, "
            + "m.deleteMillis = :deleteMillis where m.id in :id")
    ...
    @Transactional
    @Modifying
    @Query(value = "update BlackListMerchantUrl m "
