...
public interface BlackListPanRepository
    ...
    boolean existsByPanIdAndDeleteFlagFalse(long panInfoId);
    ...
    @Query(
        value =
            "update BLACK_LIST_PAN b set b.DELETE_FLAG = 1, b.DELETER = :deleter, b.DELETE_MILLIS = :deleteMillis\n"
    ...
    @Transactional
    @Modifying
    @Query("update from BlackListPan set deleteFlag = true, deleter = :deleter, deleteMillis = :deleteMillis "
        + "where panId = :panInfoId and deleteFlag = false")
    int deleteByPanInfoId(@Param("panInfoId") long panInfoId,
        @Param("deleter") String deleter, @Param("deleteMillis") long now);
}
