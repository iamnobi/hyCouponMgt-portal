...
public class TransactionLogDAOImpl implements TransactionLogDAO {
    ...
    private final ocean.acs.models.sql_server.repository.TransactionLogRepository repo;
    ...
    public TransactionLogDO save(TransactionLogDO transactionLogDO) throws DatabaseException {
        try {
            ...
            // NOTE: plugin 如果傳 "空字串" 過來，在 SQL Server 會發用 unique index exception
            if (StringUtils.isEmpty(transactionLog.getAcsIntegratorTransID())) {
                transactionLog.setAcsIntegratorTransID(null);
            }
    ...
    @Override
    public void updateNeedResendRReq(Long id, boolean needResendRReq, Long updateMillis,
        String sysUpdater) throws DatabaseException {
        try {
            repo.updateNeedResendRReq(id, needResendRReq, updateMillis, sysUpdater);
        } catch (Exception e) {
            log.error(
                "[updateNeedResendRReq] unknown exception", e);
            throw new DatabaseException(ResultStatus.DATABASE_ERROR, e.getMessage(), e);
        }
    }
