...
public interface BlackListIpGroupRepository
    ...
    @Query("select b from BlackListIpGroup b where b.id in ?1 and b.deleteFlag=0")
    List<BlackListIpGroup> findByIdIn(List<Long> ids, Pageable pageable);

    @Transactional
    @Modifying
    @Query("update BlackListIpGroup i set i.deleteFlag = 1, i.deleter = :deleter, i.deleteMillis = :deleteMillis where i.id = :id")
    ...
    @Transactional
    @Modifying
    @Query("update BlackListIpGroup m set m.transStatus = :transStatus, m.updater = :updater, m.updateMillis = :updateMillis where m.issuerBankId = :issuerBankId and m.deleteFlag = 0")
